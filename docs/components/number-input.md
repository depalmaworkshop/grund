...
> - [ ] Extract inline implementation from the consuming application's cart
...
**Current Implementation:** Inline in the consuming application's cart - needs extraction to reusable component
...
- Extract from the consuming application's cart
...
- [ ] Update the consuming application's cart to use new component
...
### Internal Resources
- Current: the consuming application's cart
- Tokens: TBD in `src/tokens.ts` → `gds-number-input-*`
