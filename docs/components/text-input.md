...
website_path: src/app/checkout/page.tsx (inline, needs extraction)
uses_tokens: false
has_variants: true
responsive: true
...
> - [ ] Extract text input patterns from `checkout/page.tsx` shipping form
...
**Current Implementation:** Inline in `checkout/page.tsx` for shipping address fields
...
### Internal Resources
- Current: `src/app/checkout/page.tsx` shipping address form
- Tokens: TBD in `/staging/tokens-wip.ts` → `gds-text-input-*`
